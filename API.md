...
**Randomat:BalanceTeams(convert_traitors)** - Changes jester, independent, and monster roles to vanilla innocents and traitors so that the ratio of innocents to traitors is as balanced as possible. (As determined by server config or role pack config)\
*Realm:* Server\
*Parameters:*
- *convert_traitors* - Whether to convert special traitors (e.g. the Hypnotist) to be a vanilla Traitor. Defaults to `true`
...
- *new_traitors* - A table of independent and monster players whose roles were changed to Traitor
...
**Randomat:PrintMessage(ply, msgtype, message, time, id, predicate))** - Prints te message to given player based on the provided type.\
*Realm:* Client and Server\
*Parameters:*
...
- *time* - The time to display this message (Only used when modern Custom Roles for TTT is installed)
- *id* - The ID used to mark this message, useful for deduplication (Only used when modern Custom Roles for TTT is installed)
- *predicate* - The predicate used to determine whether to display a message (Only used when modern Custom Roles for TTT is installed)
