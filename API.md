...
**MinPlayers** - The minimum number of players that must be in the server before this event can be chosen. This changes the default value for the created `ttt_randomat_{EVENT_ID}_min_players` convar. Changing this after the event has already been loaded on a server will only change the default convar value, not the current value. Defaults to `0`.\
*Realm:* Server
