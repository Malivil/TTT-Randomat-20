net.Receive("ElectionNominateBegin", function()
    ...
    frame:SetTitle("Nominate Presidents")
    ...
    for _, v in pairs(player.GetAll()) do
        -- Don't allow dead people, spectators, and detectives to get nominated
        if v:Alive() and not v:IsSpec() and v:GetRole() ~= ROLE_DETECTIVE then
...
net.Receive("ElectionVoteBegin", function()
    ...
    frame:SetPos(10, ScrH() - 800)
    frame:SetSize(200, 300)
    frame:SetTitle("Vote for President")
    frame:SetDraggable(false)
    frame:ShowCloseButton(false)
    frame:SetVisible(true)
    frame:SetDeleteOnClose(true)
